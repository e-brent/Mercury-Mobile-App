...
// Screen component
const ReciprocityScreen = ({route}) => {

  // Reference to use to automatically scroll down to see results
    const endRef = React.useRef();

  // State variables for saving data and updating the screen
    const [selectedIndex, setSelectedIndex] = React.useState(route.params.tab);     // Determines whether to go to Pinhole or normal Reciprocity tool
    ...
    const calculateReciprocity = (pinhole, film, seconds) => {
      ...
      <SafeAreaView style={[(timerEnd == false) ? reciprocityStyle.containerRegular : reciprocityStyle.containerTimerEnd]}>
        <ScrollView ref={endRef} onContentSizeChange={() => endRef.current.scrollToEnd({ animated: true })}>
          {/*Page titles -- the one displayed depends on the value of the state variable controlled by the segmented control tab*/}
            ...
            {selectedIndex == 1 && (<Text style={reciprocityStyle.textTitle} accessible={true} accessibilityLabel="Reciprocity" accessibilityRole="text">Reciprocity</Text>)}

          {/*Instructions -- the one displayed depends on the valu eof the state variable controlled by the segmented control tab*/}
          {selectedIndex == 0 && (<Text style={reciprocityStyle.text} accessible={true} accessibilityLabel="Meter for f/45.  Select your pinhole size, film stock, and the exposure time your meter calculates, and we will calculate your actual exposure time (taking into account your pinhole and film reciprocity)." accessibilityRole="text">
         ...
         {selectedIndex == 1 && (<Text style={reciprocityStyle.text} accessible={true} accessibilityLabel="When shooting long exposures (over 1 second), use this calculator to convert your metered exposure to the actual exposure time required by your film stock." accessibilityRole="text">
          ...
          <SegmentedControlTab
            values={['Pinhole', 'Reciprocity (Long Exposure)']}
            ...
            accessible={true}
            accessibilityLabels={['pinhole', 'reciprocity (long exposure)']}
          />
          ...
          <View style={reciprocityStyle.contentBlock}>
            ...
            <TextInput
              ...
              enterKeyHint='done'
              returnKeyType='done'
              onSubmitEditing={() => calculateReciprocity(selectedPinholeSize, selectedFilm, time)}
              accessible={true}
